  ...
  class ISAType
    ...
    def to_pbx(ind = 0)
      ...
      pbxfields.each do |fld|
        ...
        when "String"
          pbx += _pbx_indent(ind) + "%s = %s;%s%s" % [fld, field, (_pbx_newline == nil ? " " : ""),_pbx_newline]
        when 'PBXTypes::BasicValue'
          pbx += _pbx_indent(ind) + "%s = %s;%s%s" % [fld, field.to_pbx, (_pbx_newline == nil ? " " : ""), _pbx_newline]
        when "Array"
